...
<div class="contentBox contentTablelist allWidth">
        ...
        <div class="spanLineTableContent">
            ...
            <div class="spanLine tablelist">
                <?php echo form_open("admin/$child1_name_Str/$child2_name_Str/$child3_name_Str/{$child4_name_Str}_post/") ?>
                    ...
                    <?endif?>
                    <div class="spanLineLeft text width100">
                        <input type="number" class="text" style="margin-left:-6px;" value="<?=!empty($search_faqid_Num)?$search_faqid_Num:''?>" name="search_faqid_Num" placeholder="請填寫ID">
                    </div>
                    <div class="spanLineLeft text width500">
                        <input type="text" class="text" style="margin-left:-6px;" value="<?=!empty($search_title_Str)?$search_title_Str:''?>" name="search_title_Str" placeholder="請填寫問題標題">
                    </div>
                    <div class="spanLineLeft text width150">
            ...
            <div class="spanLine tablelist">
                <div class="spanLineLeft checkbox">
                    <input type="checkbox" name="faqid_Arr[]" value="<?=$value_Faq->faqid_Num?>" class="check">
                </div>
                <div class="spanLineLeft text width100">
                    <?=$value_Faq->faqid_Num?>
                </div>
                <div class="spanLineLeft text width500">
                    <a href="admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/edit/?faqid=<?=$value_Faq->faqid_Num?>"><?=$value_Faq->title_Str?></a>
                </div>
                <div class="spanLineLeft text width150">
                ...
                <div class="spanLineLeft width150 tablelistMenu">
                    <a href="admin/<?=$child1_name_Str?>/<?=$child2_name_Str?>/<?=$child3_name_Str?>/edit/?faqid=<?=$value_Faq->faqid_Num?>">編輯</a>
                </div>
            </div>
            <?endforeach?>
            <?else:?>
    ...
    <?if(!empty($FaqList->obj_Arr[0]->faqid_Num)):?>
    <div class="batch_deletion">
        <input type="submit" class="button" style="height: 32px;" onclick="return confirm('刪除後將進入回收空間，確定要刪除嗎？')" value="刪除常見問題">
        <span class="delete_prompt">可以選取單篇常見問題或多篇常見問題刪除<span>
    </div>
    <?endif?>
    </form>
